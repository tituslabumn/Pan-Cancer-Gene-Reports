...
```{r echo=FALSE, fig.width=10,out.width='\\textwidth'}
...
a<-lolliplot(SNP.gr = F2_variants,
             features = F2_features,
             ylab = "Filteres ensembl transcript exons",
             xaxis = F2_x_axis,
             yaxis = FALSE)
grid.text("Filtered Ensembl Transcripts (Union of Exonic sequence)",
          x=.5,
          y=0.9,
          just="top", 
...
```{r echo=FALSE, fig.width=10, fig.height=10,out.width='\\textwidth'}
...
```{r echo=FALSE}
kable(overlap_summary_table, row.names = FALSE, caption = "All variants shared between cBP and gnomAD")
```
