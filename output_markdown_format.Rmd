...
```{r echo=FALSE,fig.height=6, fig.width=10,out.width='\\textwidth',strip.white=TRUE}
F1_magick_img <- image_graph(width = 1500, height = 1000,res = 150, clip = FALSE)
...
```{r echo=FALSE,fig.height=6,fig.width=15,out.width='\\textwidth',strip.white=TRUE}
F2_magick_img <- image_graph(width = 2200, height = 1000,res = 150, clip = FALSE)
lolliplot(SNP.gr = F2_variants,
             features = F2_features,
             ylab = "Filteres ensembl transcript exons",
             xaxis = F2_x_axis,
             yaxis = FALSE)
dev.off()
...
```{r echo=FALSE, fig.asp=1, fig.width=15,out.width='\\textwidth'}
lolliplot(SNP.gr = F3_variants ,
          ...
          jitter = "label",
          legend =  legend)
grid.text(paste0("All ",GOI," cBioPortal variants (n = ",length(F3_variants$score),")"),
          x=0.5,
          y=0.8,
          just="top", 
